...
class GenericCSVReader:
    def __init__(self, config: dict, output_queue, shutdown_event):
        self.config = config
        ...
        # Load crypto settings dynamically from config
        self.crypto_cfg = self.config["processing"]["stateless_tasks"]
    ...
    def run(self):
        ...
        try:
            with open(self.file_path, mode='r', encoding='utf-8') as file:
                ...
                for row in reader:
                    ...
                    if metric is None or not isinstance(metric, (int, float)):
                        print(f"[INPUT WARNING] Skipping row {current_sequence_id}: metric_value is missing/invalid.")
                        continue

                    raw_value_str = f"{metric:.2f}"
                    signature = generate_signature(raw_value_str, self.secret_key, self.iterations)
            ...
            print(f"[INPUT ERROR] Critical error reading file: {e}")
